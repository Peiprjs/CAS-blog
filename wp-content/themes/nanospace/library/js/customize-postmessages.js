...
(function( $ ) {
	...
	var nanospaceCustomizer = {

		/**
		 * PostMessage: Output CSS.
		 */
		postMessage_css : function( key, rules, newValue ) {
			var styleID = 'nanospace-customize-preview-css-' + key,
			    $style = $( '#' + styleID ),
			    css = '',
			    cssArray = {};
		...
		postMessage_font : function( key, rules, newValue ) {
			...
			if ( -1 < newValue.indexOf( '|' ) ) {
				...
				if ( 2 === chunks.length && undefined !== nanospaceCustomizerPreviewData.fonts[ chunks[0] ] ) {
					fontSource = chunks[0],
					fontName = chunks[1],
					fontStack = nanospaceCustomizerPreviewData.fonts[ fontSource ][ fontName ] || '';
				}
			}

			// Change CSS value.
			nanospaceCustomizer.postMessage_css( key, rules, fontStack );

			// If value is from Google Fonts, update the embed link.
			if ( 'google_fonts' === fontSource ) {
				// Get the <link> tag.		
				var googleFontsLinkID = 'nanospace-customize-preview-google-fonts-css-' + key,
	...
	if ( undefined !== nanospaceCustomizerPreviewData ) {
		if ( undefined !== nanospaceCustomizerPreviewData.postMessages ) {
			_.each( nanospaceCustomizerPreviewData.postMessages, function( postMessages, key ) {
				...
				wp.customize( key, function( setting ) {
					setting.bind(function( value ) {
						_.each( sortedPostMessages, function( rule, type ) {
							...
							nanospaceCustomizer[ functionName ]( key, rule, value );
						});
					});
				});
			});
